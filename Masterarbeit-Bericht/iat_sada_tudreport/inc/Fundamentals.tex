...
Edge detectors are essential parts of most computer vision systems. Edge detectors dramatically decrease the amount of data to be processed and extract the useful parts of images. They work by detecting discontinuities in brightness. \emph{\color{blue} It is usually done by applying a 2-D filter, which is designed the way it is only sensitive to the large gradinents present in the image, returning zero values for all monotonous regions. There is a big variety of specificly composed operators, each of them showing the best results when used with some particular types of edges. For instance, different orientation of the edges, their geometry, structure, and thickness should correspond with operator characteristics in order for the detection to be reliable. Another problem is the image noise, which in many cases could be confused with local gradient and thus detected as an edge. Because of that, there is no universally efficient operator, for each particular case it should be specifically configured, and some pre-editing must be used in order to achieve the best detection quality.} In this project, the edge detector is used to detect the lanes and to exclude other unnecessary information from images. There are several different methods suitable for lane edge detection, and all of them can be grouped into two categories. They are :

\begin{enumerate}

\item \textbf{Gradient method : } This method searches for the maximum and minimum in the first derivative of the image and with that, the edges can be found. \emph{\color{blue} From the color intensity perspective, any edge has either rise or fall, which gives a local spike after applying derivative operator, hence highlighting the edge location in the image. } For this method, the first order derivative filter must be used. For example : Sobel-Operator, which is described in detail in Section \ref{sec:Sobel Operator}.
 
\item \textbf{Laplacian method : } This method searches for the zero crossing in the second derivative of the image. \emph{\color{blue} This is an alternative for locating local spikes, as the second derivative is equal to zero when the first is at minimum or maximum. } For example : Canny edge detector, which is described in detail in Section \ref{sec:Canny Edge Detector}. 
...
\emph{\color{red}The main disadvantage of Canny edge detector is that it is time consuming, due to its complex computation. }
...
\emph{\color{blue}In this master's thesis the Sobel Operator was utilized. The first reason for using it instead of Canny Edge Detection algorithm is its performance. The latter uses second derivative, which is computationally harder and takes more time (see table ?? for timing comparison). For the purpose of lane detection it is crucial to spend as little computation time per frame as possible, because longer timings may result in substantial delay between the image being captured and the car driving module reacting to a new lane position. }

//insert table with timings for sobel and canny here

\emph{\color{blue}The other reason is that in case of poor light conditions Canny Edge Detector tends to filter out parts of the lane lines, whereas Sobel Operator shows much better results (see picture ??)}
...
In this equation, \textit{$ \rho $} is the length of a normal from the origin to the line and \textit{$ \theta $} is the orientation of \textit{r} with respect to the x-axis.
 ...
 \emph{\color{blue} Standard Hough Transformation algorithm consists of the following steps: }
...
\begin{algorithmic}[1]
 ...
 \emph{\color{blue} An edge detection algorithm is applied to the image with dimensions $I_{w}, I_{h}$, which provides a binary data used at step 4 to determine whether a pixel belongs to an edge. Steps 3 - 7 convert detected edges to the Hought space. The following function is used to calculate $\overline{\rho}$ for all detected edges that pass through the point $(x,y)$ at angle $\overline{\theta}$: }
...
$ \overline{\rho}(\overline{\theta},x,y) = \Bigg[\dfrac{H_{\rho}\big((y-\dfrac{I_{h}}{2})sin(\dfrac{\pi}{H_{\theta}}\overline{\theta})+(x-\dfrac{I_{w}}{2})cos(\dfrac{\pi}{H_{\theta}}\overline{\theta})\big)}{\sqrt{I^{2}_{w}+I^{2}_{h}}}\Bigg] $ 
 \end{center}
  ...
  \emph{\color{blue} At step 8 the local maximum of all accumulated lines is determined, which is then transformed from discretized cordinates $\overline{\rho}$ and $\overline{\theta}$ into $\rho$ and $\theta$ using the following formulas:  } 
...
Essentially, the point of the Hough Transformation is to implement a 'voting procedure' for all potential curves in the image. Curves that actually found in the image will then have relatively high 'voting scores' when the algorithm terminates. Probabilistic Hough Transformation differs from Standard Hough Line Transformation by trying to avoid a computationally expensive 'voting process' for every nonzero pixel in the image. It does this by taking advantage of the geometric properties of analytical curves, which allows it to improve efficiency as well as reduce the storage requirement of the original algorithm.
 ...
 \begin{figure}[H]
 \centering
  \includegraphics[width=0.9\textwidth]{./Bilder/Probabilistic_HT.png}\label{Standard_Hough_Line_Transformation_fig}
  \caption{Standard Hough Line Transformation\cite{Standard_Hough_Transformation}}
\end{figure}

\emph{\color{blue} Line detection using RHT is similar to standard Hough Transformation but instead of using all edge points of the image some subset is randomly selected, which is then used to determine a single unique parameter space point for every pair of points from the subset (see picture ??).}

\emph{\color{blue}As soon as the amount of points that are mapped to the same parameter in Hough space exceeds a certain threshold, the line is confirmed to be present in the original image, i.e. the line is detected. It is then removed from the input array so that the other lines are detected faster. The algorithm reaches the termination either after all the lines are detected and there is no more input data, or the maximum limit of iterations is reached. Because of this approach, the required computational power and memory consumption are much less compared to standard Hough Transformation.}


RHT  algorithm consists of the following steps:
