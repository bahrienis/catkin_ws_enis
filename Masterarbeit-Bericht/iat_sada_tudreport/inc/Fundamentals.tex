...
The parameters of the function will be described in detail.\ref{GaussianBlur}
...
\emph{\color{blue}Last step of Sobel operator is approximating the gradient by adding both directional gradients. One step ago, gradients of x and y coordinates calculated separately. With following command, weighted sum of these two gradients must be calculated.\ref{addWeighted}}
...
\emph{\color{blue}Canny Edge Detector is also a function in OpenCV. As Sobel Operator, before Canny Edge Detector is used, a filter should be used(like Gaussian Filter) and the image must be converted to gray scale.}
