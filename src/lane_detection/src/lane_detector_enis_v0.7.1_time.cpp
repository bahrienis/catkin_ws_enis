//IPM-Resize-Lane Detection-k nearest neighbour
...
clock_t end_pre = clock();
        ...
        Rect Rec1(0, 2*heightofframe/3, widthofframe, heightofframe/3);
...
clock_t end_SHT = clock();
        ...
        for (int i = 0; i < numOfRedLinesUp; i++) {
            if (findredlinesarray1[i] - findredlinesarray1[i + 1] > widthofframe/25) {
                midLineIndex = i + 1;
                break;
            }
        }
