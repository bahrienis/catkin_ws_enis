...
void curvefitting(int numofpointsoffirstlane2, int numofpointsoffirstlane1,
        double *y, double *x, Mat inputImg, string color, double dWidth, double dHeight, Mat blacknewimg2);
...
int main(int argc, char **argv) {
    ...
    double dWidth = 640; //get the width of frames of te video
    ...
    for (;;) {
        ...
        Mat imageblack(480, 640, CV_8UC3, Scalar(0, 0, 0));


        Mat blackoutputImg(480, 640, CV_8UC3, Scalar(0, 0, 0));
  ...
  //      std::string filename = "/home/enis/Desktop/Masterarbeit/photos_04.09.2017/frame140.jpg";
       ...
       inputImg = imread(filename, CV_LOAD_IMAGE_COLOR);
        ...
        for (int firstcolor = 0; firstcolor < (numofpointsoffirstlane2 + numofpointsoffirstlane1); firstcolor++) {
            imageblack.at<cv::Vec3b>(y1[firstcolor], x1[firstcolor])[0] = 255; //turn the pixel value @ (k,i) to yellow (0,255,255)
            imageblack.at<cv::Vec3b>(y1[firstcolor], x1[firstcolor])[1] = 0;
            imageblack.at<cv::Vec3b>(y1[firstcolor], x1[firstcolor])[2] = 0;
        ...
        for (int secondcolor = 0; secondcolor < (numofpointsofsecondlane2 + numofpointsofsecondlane1); secondcolor++) {
            imageblack.at<cv::Vec3b>(y2[secondcolor], x2[secondcolor])[0] = 0; //turn the pixel value @ (k,i) to yellow (0,255,255)
            imageblack.at<cv::Vec3b>(y2[secondcolor], x2[secondcolor])[1] = 255;
            imageblack.at<cv::Vec3b>(y2[secondcolor], x2[secondcolor])[2] = 0;
        ...
        for (int thirdcolor = 0; thirdcolor < (numofpointsofthirdlane2 + numofpointsofthirdlane1); thirdcolor++) {
            imageblack.at<cv::Vec3b>(y3[thirdcolor], x3[thirdcolor])[0] = 0; //turn the pixel value @ (k,i) to yellow (0,255,255)
            imageblack.at<cv::Vec3b>(y3[thirdcolor], x3[thirdcolor])[1] = 0;
            imageblack.at<cv::Vec3b>(y3[thirdcolor], x3[thirdcolor])[2] = 255;
...
Mat blacknewimg2(480, 640, CV_8UC3, Scalar(0, 0, 0));




        curvefitting(0, numofpointsoffirstlane1new, y11, x11, imageblack, "blue", dWidth, dHeight, blacknewimg2);

        curvefitting(0, numofpointsofsecondlane2new, y22, x22, imageblack, "green", dWidth, dHeight, blacknewimg2);
        
        curvefitting(0, numofpointsofthirdlane3new, y33, x33, imageblack, "red", dWidth, dHeight, blacknewimg2);
                ...
                ipm.drawPoints(origPoints, imageblack );
                ...
                Vec3b intensity = blackoutputImg.at<Vec3b>(240, 136);
...
//	fillPoly(outputImg, pts, Scalar(100,100,100,0));
        fillPoly(blackoutputImg, pts, Scalar(blue,green,red,0));
                ...
                Vec3b intensity1 = blackoutputImg.at<Vec3b>(240, 504);
uchar blue1 = intensity1.val[0];
uchar green1 = intensity1.val[1];
uchar red1 = intensity1.val[2];
        ...
        fillPoly(blackoutputImg, pts1, Scalar(blue1,green1,red1,0));	 
...
void curvefitting(int numofpointsoffirstlane2, int numofpointsoffirstlane1,
        double *x, double *y, Mat inputImg, string color, double dWidth, double dHeight, Mat blacknewimg2) {
    int i, j, k, n, N;
        ...
        for (i = 0; i < n; i++)
            ...
            for (p = 0; p < 480; p++) {
                ...
                if (r >= 0 && r <= 640) {
                    ...
                    //Point point = ipm.applyHomography(Point(r, p));
                    
                    if (color == "red") {
                        //cout << "red point coordinates: " << point << endl;
                        circle(inputImg, Point(r, p), 1, Scalar(0, 0, 255), 1, CV_AA, 0);
                        //circle(blacknewimg2, Point(point.x, point.y), 1, Scalar(0, 0, 255), 1, CV_AA, 0);
                    } else if (color == "blue"){
                        //cout << "blue point coordinates: " << point << endl;
                        circle(inputImg, Point(r, p), 1, Scalar(255, 0, 0), 1, CV_AA, 0);
                        //circle(blacknewimg2, Point(point.x, point.y), 1, Scalar(255, 0, 0), 1, CV_AA, 0);
                    } else if (color == "green"){
                        //cout << "green point coordinates: " << point << endl;
                        circle(inputImg, Point(r, p), 1, Scalar(0, 255, 0), 1, CV_AA, 0);
                        //circle(blacknewimg2, Point(point.x, point.y), 1, Scalar(0, 255, 0), 1, CV_AA, 0);
                    }
