...
int main(int argc, char **argv) {
    ...
    origPoints.push_back(Point2f(dWidth, dHeight/8));
    origPoints.push_back(Point2f(0, dHeight/8));

    // The 4-points correspondences in the destination image
    vector<Point2f> dstPoints;
    dstPoints.push_back(Point2f(dWidth / 2 - dWidth/9.6, dHeight));
    dstPoints.push_back(Point2f(dWidth / 2 + dWidth/9.6, dHeight));
...
clock_t end_pre = clock();
        ...
        Rect Rec1(0, firstpicsize+secondpicsize, dWidth, thirdpicsize);
...
clock_t end_SHT = clock();

        for (int i = 0; i < redlines.size(); i++) {
            if (i <= redlines.size() - 2) {
                if (redlines.at(i) - redlines.at(i + 1) >= dWidth/9.6) {
...
//        cout << "left Lane Index Start : " << leftLaneIndexStart << endl;
        ...
        Rect Rec2(0, firstpicsize, dWidth, thirdpicsize+secondpicsize);
        //        line(inputImg, Point(0,99),Point(640,99),Scalar(0,255,0),1,CV_AA);
        //   rectangle(inputImg, Rec1, Scalar(255), 1, 8, 0);
        //    rectangle(inputImg, Point(0, 0),Point(640, 99), Scalar(255), 1, 8, 0);
    ...
    rectangle(inputImg,Point(x_value-y_value/8,y_value),Point(x_value+y_value/8,y_value-y_value/5),Scalar(255), 1, 8, 0);
      ...
      lanePoints.push_back(houghpoints.at(i));
        ...
        waitKey(1);
                ...
                double pre_elapsed_secs = double(end_pre - begin) / CLOCKS_PER_SEC;
        ...
        double rect_elapsed_secs = double(end_rect - end_PHT) / CLOCKS_PER_SEC;
        printf("%.2f (ms)\r", 1000 * rect_elapsed_secs);
        cout << "rect : " << 1000 * rect_elapsed_secs << endl;
        
        
        

        double CF_elapsed_secs = double(end_CF - end_rect) / CLOCKS_PER_SEC;
